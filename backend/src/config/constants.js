...
const PERMISSIONS = {
  ...
  LACTATION_UPDATE: 'lactation:update',
  LACTATION_END: 'lactation:end',
  LACTATION_DELETE: 'lactation:delete',
...
const USR_PERMS = [
  PERMISSIONS.DASHBOARD_READ, PERMISSIONS.LACTATION_READ, PERMISSIONS.LACTATION_CREATE,
  PERMISSIONS.LACTATION_END, PERMISSIONS.RESERVATION_CREATE, PERMISSIONS.RESERVATION_READ_OWN,
  PERMISSIONS.RESERVATION_UPDATE_OWN, PERMISSIONS.RESERVATION_CANCEL_OWN,
  PERMISSIONS.CALENDAR_READ, PERMISSIONS.SALONES_READ,
];

const ADMIN_PERMS = [
  ...USR_PERMS,
  PERMISSIONS.LACTATION_UPDATE, PERMISSIONS.LACTATION_READ_ALL,
  PERMISSIONS.RESERVATION_READ_ALL, PERMISSIONS.RESERVATION_UPDATE_ANY,
  PERMISSIONS.RESERVATION_CANCEL_ANY, PERMISSIONS.RESERVATION_APPROVE,
  PERMISSIONS.RESERVATION_REJECT, PERMISSIONS.SALONES_CREATE, PERMISSIONS.SALONES_UPDATE,
  PERMISSIONS.SALONES_DELETE, PERMISSIONS.REPORTS_READ, PERMISSIONS.REPORTS_EXPORT,
  PERMISSIONS.USERS_READ, PERMISSIONS.SETTINGS_READ, PERMISSIONS.EMAIL_TEMPLATES_READ,
];

const SA_PERMS = [
  ...ADMIN_PERMS,
  PERMISSIONS.LACTATION_DELETE, PERMISSIONS.RESERVATION_DELETE,
  PERMISSIONS.USERS_CREATE, PERMISSIONS.USERS_UPDATE, PERMISSIONS.USERS_DELETE,
  PERMISSIONS.USERS_CHANGE_ROLE, PERMISSIONS.USERS_RESET_PASSWORD,
  PERMISSIONS.PERMISSIONS_MANAGE, PERMISSIONS.SETTINGS_UPDATE,
  PERMISSIONS.EMAIL_TEMPLATES_UPDATE, PERMISSIONS.AUDIT_READ, PERMISSIONS.AUDIT_EXPORT,
];
...
const SHEET_NAMES = {
  ...
  PLANTILLAS_EMAIL: 'plantillas_email',
};
...
// Valores por defecto. BREVO_API_KEY es NUEVA en v1.3.
// Los campos SMTP_* se conservan por compatibilidad pero ya no se usan.
const DEFAULT_CONFIG = {
  ...
  BREVO_API_KEY: '',           // ⬅️ NUEVO: API key de Brevo
  SMTP_HOST: '',               // legacy, ya no se usa
  SMTP_PORT: 587,              // legacy
  SMTP_USER: '',               // legacy
  SMTP_PASS: '',               // legacy
  SMTP_SECURE: false,          // legacy
};
